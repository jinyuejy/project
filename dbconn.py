...
def register_dsn( pool_name=None, minconn=1, maxconn=20):
    '''注册数据库连接字符串'''
    dsn="host=localhost dbname=csdb user=hopers password=hope"
    pool = psycopg2.pool.ThreadedConnectionPool(minconn=minconn, maxconn=maxconn, dsn=dsn)
...
class SimpleDataCursor(object):
    ...
    def fetchall_dicts(self, dict_type=collections.OrderedDict):
        ...
        for r in self._cursor:
            rows.append(dict(zip(field_names, r)))
        return rows
